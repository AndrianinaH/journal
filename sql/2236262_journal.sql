...
--
-- Client :  fdb13.awardspace.net
-- Généré le :  Dim 26 Mars 2017 à 17:10
...
--
-- Base de données :  `2236262_journal`
--
...
CREATE ALGORITHM=UNDEFINED DEFINER=`2236262_journal`@`%` SQL SECURITY DEFINER VIEW `allauteur`  AS  select `a`.`idArticle` AS `idArticle`,`a`.`idAuteur` AS `idAuteur`,`u`.`nomUser` AS `nomAuteur` from (`article` `a` join `user` `u` on((`a`.`idAuteur` = `u`.`idUser`))) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`2236262_journal`@`%` SQL SECURITY DEFINER VIEW `allpublieur`  AS  select `a`.`idArticle` AS `idArticle`,`a`.`idPublieur` AS `idPublieur`,`u`.`nomUser` AS `nomPublieur` from (`article` `a` join `user` `u` on((`a`.`idPublieur` = `u`.`idUser`))) ;
